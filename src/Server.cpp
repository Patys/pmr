...
void Server::setup()
{
	...
	world.addEnity(Enity(sf::Vector2f(300,300),
					sf::Vector2f(64,64),
					100, true, "bush1", "bush_01"));
	world.addEnity(Enity(sf::Vector2f(200,200),
					sf::Vector2f(64,128),
					100, true, "tree1", "tree_01"));
	world.addEnity(Enity(sf::Vector2f(250,300),
					sf::Vector2f(64,128),
					100, true, "tree1", "tree_02"));
	world.addEnity(Enity(sf::Vector2f(300,50),
					sf::Vector2f(64,128),
					100, true, "tree1", "tree_03"));
	world.addEnity(Enity(sf::Vector2f(400,100),
					sf::Vector2f(64,128),
					100, true, "tree1", "tree_05"));
	world.addEnity(Enity(sf::Vector2f(450,220),
					sf::Vector2f(64,128),
					100, true, "tree1", "tree_06"));
	world.addEnity(Enity(sf::Vector2f(500,180),
					sf::Vector2f(64,128),
					100, true, "tree1", "tree_07"));

	world.addEnity(Enity(sf::Vector2f(300,200),
					sf::Vector2f(64,64),
					100, false, "axe1", "axe_01"));

	world.addEnity(Enity(sf::Vector2f(350,200),
					sf::Vector2f(64,64),
					100, false, "sword1", "sword_01"));
}
...
void Server::excuteCommand(const std::string& command, std::vector<std::string> data, sf::TcpSocket* client)
{
	...
	if(command == "pick up")
	{
		...
		if(player)
		{
			...
			{
				auto item = world.getEnity(item_id);
				if(item)
				{
